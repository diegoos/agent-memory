...
- sensitive: `instructions.md` — Validation max 5 bullets (closure command; historical reviews in `log.md`). **Cold session** four questions are the write quality bar; Q4 is the **done signal**. Resume rotten also when Validation is `_none_` / has no exact command. User constraint includes a flipped lasting assumption; this-branch flips go on active-work Assumptions. Slim: write floor is the gate; format lives under _How to write_; start/stop point at those headings (same behaviour). Show diff and confirm.
