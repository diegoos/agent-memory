...
- safe: `agent-memory-print-evidence.sh` — allowlisted hook-state stdout (`pending_count` / hex HEAD / validated session id / sanitized branch). Never prints path lists. `SKILL.md` `allowed-tools` lists exact `Bash(…print-evidence.sh)` paths beside consume-evidence.
...
- sensitive: **graph reshape** (`references/update-graph.md`) — collapse leftover `vision.md` / `architecture.md` / `patterns.md` / `domains/*` / `features/*` / graph-tree dirs; rewrite Recall files; rewrite dead links and unlinked `vision.md` / domain prose; insert missing `**Status:** live` on dated `decisions.md` headings; rewrite illegal log `[type]` to `chore`; merge same-day same-`[type]` `log.md` headings; delete non-md `active-work` junk and closed-placeholder resume files (confirm). **Do not** merge project-docs links into `index.md` Canonical sources (`AGENTS.md` is that map). Runs even when `.version` already equals latest. **Supersedes 0.0.14** “Do not delete legacy mirror files.” Do not invent learnings or `when editing:` globs.
- safe: `SKILL.md` — `update` allowed-tools include `decisions.md` and mirror paths for graph reshape only; `AGENTS.md` for docs-map patches (`references/docs-map.md`); sync still must not write decisions/learnings. Help/routing name mirror delete + AGENTS map. After the loaded command's Report, do not append a Memory line. Always-on block names triggers; _Harness parity_ stays a pointer into `instructions.md`.
- sensitive: `index.md` / `instructions.md` — Canonical sources list a path only when `AGENTS.md` does not already link it and a cold session needs it before choosing a recall file; missing `docs/` / ADR trees stay unlisted; superseded decisions collapse to heading + Status + `Superseded by:`; no ADR-index pointer in memory when the tree is absent. Catch-up and harness parity: print-evidence / consume-evidence; do not Read `.hook-sync-state`. Method rules for learnings H2 and the AGENTS docs map live in-file. Show diff and confirm. Preserve project-specific notes.
- safe: `update` / `init` — `.agents/memory/.version` stamps this skill's `SKILL.md` `metadata.version` (not a drafted `UPDATE.md` heading newer than that string). Last `## <version>` in `vendor/UPDATE.md` equals metadata. Do not stamp downward when installed is a later SemVer. Tokens `instructions` / `agents.md` / `rules` insert or refresh the canonical block in `AGENTS.md` only. Bare `init` still refuses an existing `.agents/memory/` except those tokens. Full `update` skip of a missing block on other carriers points at `update instructions`.
- safe: `update` / CLI — classify each installed harness via `$hooksDir/.version` vs skill `metadata.version` plus the five shared scripts (OpenCode: plugins + `safe-script.ts`). **current** when Complete and stamp SemVer is equal or later; **stale** when the stamp is missing, incomplete, or older. Print installer commands only for stale. CLI `update` skips hooks when the stamp is later than the package (`--force` and source checkout included). Skill install resolves a symlink project root with `realpath`, same as `install-hooks.sh`. Installed-harness detection uses `HARNESS_HOOKS_DIR`. Report may offer `/agent-memory lint` (optional). When installed SemVer is ahead of the skill, `.version` stays; always-on gitignore / graph reshape / blocks still run.
- safe: `lint` — findings `decision-canonical-dup` / `decision-docs-map` / `decision-stale-live` / `incident-unpromoted` (per heading) / `decision-hidden` (aggregate: any live decision names `src/` or a `*.config.*` path while the index `decisions.md` line lacks `when editing:`) / `decision-lesson-dup` (Relates on the decision **or** a learnings `decisions.md#slug` back-link; not prose H2 mentions) / `closed-placeholder-resume` / `wrong-cwd:` (structural script not run from the documented cwd — memory: `.agents/memory/`; root: project root); `dup-progress-log` at >5 Progress bullets; `dup-validation:` at >5 Validation bullets; `live-dup-identity` docs-layout heuristic (`reorganiz` / `docs layout` / `docs suite` / `docs/specs` / `docs tree`); `memory-ghost-docs:` on live backtick `docs/…`. Overbroad includes `src/pages/**`, covering `src/pages/**…` and `src/<seg>/**`. `relates-missing` `#fragment` is a heading slug (cached per target). Root `stale-resume` accepts any valid `Checkpoint:` matching HEAD. Orphan basename probes use literal `grep -F`. `decisions.md` **non-empty** > 200 maps to consolidate Pass A (Reference), not a learnings split. Structural emitters live in skill `scripts/` (`references/lint-structural.md`); fence-aware scans for `missing:`, typed Relates, `log.md` `## Format` body, `legacy-path-bullet`, and root `memory-ghost-docs`; `bad-log-heading:` / `empty-log-heading:` in the orchestrator. Learnings slug edges count only on `- Relates:` lines. Resolve skill-dir to an absolute path, then run `<absolute-skill-dir>/scripts/lint-structural-from-memory.sh` from `.agents/memory/` and the root script from the project root.
- safe: `lint --fix` may refresh `stale-resume:` Checkpoint (today @ HEAD short SHA), trim `dup-progress-log:` / `dup-validation:` to ≤5 bullets, and delete closed-placeholder resumes that are not the current git branch (confirm). `overbroad-hint:` / `live-dup-identity:` / `decision-lesson-dup:` / `memory-ghost-docs:` stay consolidate Pass A (default Apply). `--fix` does not slim `decisions.md`.
- sensitive: `consolidate` — two passes: **A corpus** (always) and **B closed log**. Report **no-op** only when both are empty. Open branch ≠ current session. Empty Pass A while those lint IDs exist is a failed consolidate. Pass A default **Apply** (Defer only with missing-evidence reason). Pass A Apply on live `memory-ghost-docs`; `decisions.md` **non-empty** > 200 is a candidate even with no other ID; second Pass A in the same run until non-empty ≤ 200 or remaining live headings are pointer-only (blank lines do not count; Report `decisions.md non-empty N (budget 200)`; leftover live Context/Why while N > 200 is a failed consolidate). Pass A also deletes `double-injection:` / `delegation-canary:` redundant blocks. Prior-day log headings default **Trim**. At most 3 Learning promotions per run, from incident-shaped decisions only (not log diary); one existing H2 does not skip other `incident-unpromoted` headings. Collapse `decision-lesson-dup`. Delete `closed-placeholder-resume` when not the current git branch. Pass B trims `dup-validation:`. Show plan; confirm each diff.
- sensitive: `instructions.md` — catch-up print-evidence; live `Source:` to spec/ADR stays a pointer; live + “superseded by” is a bug; identity is the approach noun; `when editing:` hints are Evidence path literals; in-turn Reusable lesson stays fail-closed; `/agent-memory consolidate` may promote incident-shaped decisions already on disk. Validation max 5 bullets (closure command; historical reviews in `log.md`). **Cold session** four questions are the write quality bar; Q4 is the **done signal**. Resume rotten also when Validation is `_none_` / has no exact command. User constraint includes a flipped lasting assumption; this-branch flips go on active-work Assumptions. Duplicate rule: equivalent Insight still skip; opposite Insight in the target file → `- Relates: contradicts` on the new H2. Slim: write floor is the gate; format lives under _How to write_; start/stop point at those headings. Write-floor **Winner** (first yes): User constraint → Reusable lesson → Closed why missing → Resume rotten → Shared blocker. Show diff and confirm. Preserve project-specific notes.
- sensitive: harness agent-memory block + `instructions.md` _When stopping_ — last assistant line `Memory: <file>` only when this turn wrote memory (not a write). Skip prints no Memory line. Injected block names the same Winner order. Show block diff; confirm.
...
- safe: skill flag tokens — `--fix`/`fix`, `--auto`/`auto`, `--force`/`force` are the same. A leading `--` stays valid. `learn` / `consolidate` reject both `--auto` and `auto`.
- safe: hooks — five shared scripts; `hooks/agent-memory-hooks/` derives install-site like harness dirs (stale `*_PROJECT_DIR` / `GITHUB_WORKSPACE` must not retarget state). Status `when editing:` uses lint Overbroad denylist (`src/pages/**…` / `src/<seg>/**`; `*` does not cross `/`). Sync checkpoint lists tracked dirty files only. Session-id fallback binds the first `"session_id"` key. End-of-turn stderr resume nudge when pending paths, Checkpoint behind HEAD, or dirty tree with no `active-work` (never Markdown; never `followup_message`; not on sessionStart / preCompact / precommit). **User must re-run the hooks installer** (shared scripts).
