...
- safe: `sync` / `learn` / `lint` / `consolidate` / `bootstrap` references — meaning sources; **required** consume when eligible; learn skip report + `when editing:` from Evidence; `evidence-stale-uncleared` / `pending-doc-met` / `checkpoint-prose` / `stale-next-step` / `dup-progress-log`; skip dogfood instructions↔vendor `dup-exact`; mixed-type log / met pending-doc consolidate; bootstrap Next step product-only.
- sensitive: `instructions.md` — must-consume on primary-write/sync; Next step product-only; Checkpoint line hygiene; Progress vs log. Show full diff and confirm. Preserve project-specific notes.
