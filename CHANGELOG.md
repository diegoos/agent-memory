...
Human release notes. Machine migrations (`safe` / `sensitive`) live in [`skills/agent-memory/vendor/UPDATE.md`](skills/agent-memory/vendor/UPDATE.md). Keep both aligned on version bumps.
...
- Write floor: one file or skip. A git commit does not skip the floor. Cold session is four questions; Validation is the done signal. Resume rotten also when Validation has no exact command. A flipped lasting assumption is a user constraint. Always-load is `index.md` and `current.md`; `AGENTS.md` is the docs map. After a turn that wrote memory, last line `Memory: <file>`. Skip prints nothing. After a skill command, no Memory line. Opposite Insight: `- Relates: contradicts` on the new H2. Winner (first yes): User constraint, then Reusable lesson, then closed why, then resume rotten, then shared blocker.
