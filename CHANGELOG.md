...
- `/agent-memory update` **graph reshape** collapses leftover `vision.md` / `architecture.md` / `patterns.md` / `domains/*` / `features/*` into `index.md` pointers, inserts missing `Status: live` on decisions, merges same-day same-type log headings, and deletes junk/closed-placeholder active-work. Unique facts stay. Dead links are rewritten. Confirm each diff. Does not invent learnings. Re-run `update` on installs already at `0.2.1-rc.0` / `0.2.1-rc.1` so leftover mirrors go.
