...
Migration details for `/agent-memory update` live in [`skills/agent-memory/vendor/UPDATE.md`](skills/agent-memory/vendor/UPDATE.md) (`safe` / `sensitive` tags for machines). This file is the release history for humans. Keep both in sync on version bumps.

## [Unreleased]

### Added

- `/agent-memory init instructions` and `/agent-memory update instructions` (also `agents.md`, `rules`) insert or refresh the delimited agent-memory block in `AGENTS.md` when memory already exists or as part of init. They do not add a separate `install` command.
- Skill flag tokens: `--fix` / `fix`, `--auto` / `auto`, `--force` / `force` are the same. `lint --fix` and `sync --auto` stay valid. `learn` and `consolidate` reject both `--auto` and `auto`.
- Structural lint `overbroad-hint:` when an `index.md` `when editing:` glob is on the Overbroad denylist (`src/pages/**` and the rest) **or** a covering tree (`src/pages/**/index.astro`, `src/pages/blog/**`, `src/modules/**`). Consolidate Pass A replaces those globs with Evidence path literals (default Apply). Companions do not redeem a denylist glob.
- `dup-validation:` when `## Validation` has more than 5 bullets. `lint --fix` (confirm) keeps the closure command and expected result; historical reviews become at most one `log.md` pointer. Consolidate Pass B trims the same finding.
- Git `post-commit` stamps `last_processed_head` to the new HEAD and drops `session_touched_files` entries that are in that commit and no longer dirty. It does not write Markdown or block the commit, and it fails open. Copy it next to `pre-commit` (refresh `.git/hooks/post-commit` and the shared scripts).
- Recall hop: hop only on durable why (decision / successor / conflict between recall facts) when there is no path hit (diff, named file, failing test, `when editing:`, active-work). A failing test or this diff uses hints and code, not hop. Listing `log.md` on the index is not a stop. The user does not have to say the closed verb names. At most two typed links and four extra files; hop 2 is the causal set `{caused_by, supersedes, superseded_by}`. Hop 1 inbound search: `rg -nE 'Relates:|caused_by:' .agents/memory`. Canonical `- Relates: <verb> [target](path)` (aliases on decisions/log). Write `Relates:` when Evidence is already a recall file. A learning (or `index.md` `link + delta`) may update the matching `index.md` `when editing:` hint in the same event. Lint `unknown-relates-verb` / `relates-missing` (file or `#fragment`) / `learning-missing-relates` / `learning-missing-evidence` / `contradicts-unlinked` / `supersede-cycle` / `current-stale-branch`. Consolidate treats Contradiction, No evidence, and Orphan Relates as prune candidates.
- Write floor (checkable skip): rotten resume → `active-work`; User constraint → `decisions.md` (live entry; supersede the prior live entry on the same identity); Reusable lesson → learnings + `index.md` `when editing:` (fail closed: incident + Git omit the why + 1-3 concrete paths); closed why missing from the commit → `log.md`; shared blocker → `current.md`. Write nothing when every row is no. A commit in Git does not skip the floor. Optional `## Hold` on active-work (max 3 bullets, lint `hold-overflow`) is branch scratch, not learnings. Superseding a live decision this turn, or adding a new learning H2, is not a HUMAN_CHECKPOINT conflicting append.
- Session Status surfaces `Next=` from active-work (one line) and `load:` recall files whose `index.md` `when editing:` globs match pending or dirty paths (including `decisions.md` and learnings). Action tells the agent to walk the write floor (reusable lesson needs incident + paths) and Read those files. Live user decisions constrain approach. Hooks do not write Markdown.
- `agent-memory-print-evidence.sh` prints allowlisted hook-state fields (`state`, `pending_count`, hex `last_processed_head`, validated `current_session_id`, sanitized `branch`). It does not print `session_touched_files`. Re-run the hooks installer so the fifth shared script is copied.
...
- `lint-structural.md` invokes the memory-cwd emitter via `<skill-dir>/scripts/…` (same as the root-cwd script), not `../scripts/` from `.agents/memory/`. `decision-lesson-dup` also fires when a learnings Relates points at a live decision that still has Context/Decision. Structural awk no longer interpolates `active-work` paths into the program text. Heading slugs transliterate diacritics (`único` → `unico`). Root-cwd AGENTS href checks use `mktemp`, not `/tmp/….$$`.
- Structural awk reads paths from `ENVIRON` (not `-v`, which interprets `\\n` in names). `relates-missing` `#fragment` matches `memory_slug` of target H2s (cached per target file). Learnings back-links match a whole slug via `decisions.md#…`, not prose H2 mentions or prefixes. `decision-hidden` fires once when any live decision names `src/` or a `*.config.*` path and the index `decisions.md` line lacks `when editing:`. Orphan basename probes use `grep -F`. Memory-cwd and root-cwd emitters emit `wrong-cwd:` when not run from the documented cwd. Root `stale-resume` accepts any valid `Checkpoint:` line that matches HEAD. Grep operands use `-e`/`--`. Lint-structural tells the agent to resolve skill-dir as an absolute path before `cd` into `.agents/memory/`.
- `tests/version-parity.sh` accepts npm prerelease versions (`0.2.1-rc.2`). CHANGELOG/UPDATE headings and GitHub pins stay on the last released tag until the RC is published. CLI `compareSemver` orders prerelease below the same core (so `0.2.1-rc.2` is not treated as equal to `0.2.1`). The last `## <version>` heading in `vendor/UPDATE.md` must equal `package.json` `version` (so a drafted `## 0.2.1` cannot sit after `## 0.2.1-rc.4` and steal the memory stamp).
- `/agent-memory update` and `init` stamp `.agents/memory/.version` from `SKILL.md` `metadata.version`, not a newer drafted UPDATE heading. They do not stamp downward when installed is a later SemVer than the skill (RC consumer that already wrote `0.2.1`). The update report names that mismatch: stable-core migrations will not run; always-on gitignore, graph reshape, and instruction blocks still run.
- `lint --fix` may refresh `stale-resume:` Checkpoint (today @ HEAD) and trim `dup-progress-log:` / `dup-validation:` to ≤5 bullets (confirm). It still does not slim `decisions.md` or rewrite overbroad index globs.
- Vendor `hooks/agent-memory-hooks/` is an install-site root (same stale-env preference as `.cursor/hooks`). Session Status `when editing:` matching uses the lint Overbroad denylist (including covering `src/pages/**…` / `src/<seg>/**` trees) and gitignore-style `*` (does not cross `/`). **User must re-run the hooks installer** (shared scripts).

### Security

- `/agent-memory sync` and consolidate must not Read `.hook-sync-state`. They run `agent-memory-print-evidence.sh` so path lists from hooks (and forged state) stay out of the model context (Snyk W011). Missing helper means no hook evidence, not a Read fallback.

### Changed

- CLI source lives under `src/` (`src/cli.ts` entry; helpers colocated). `src/cli.ts` presence marks a source checkout for same-SemVer refresh; published packs omit `src/`.
- `/agent-memory update` graph reshape deletes leftover `vision.md` / `architecture.md` / `patterns.md` / `domains/*` / `features/*`, inserts missing `Status: live` on decisions, rewrites illegal log `[type]`, merges same-day same-type log headings, and deletes junk/closed-placeholder active-work. It leaves docs links off `index.md` (`AGENTS.md` is the project-docs map). Unique non-docs facts stay. Dead links and leftover `vision.md` prose are rewritten. Confirm each diff. Does not invent learnings. Re-run `update` on installs already at `0.2.1-rc.0` / `0.2.1-rc.1` / `0.2.1-rc.2` / `0.2.1-rc.3` / `0.2.1-rc.4` so leftover mirrors go.
- `/agent-memory lint` is six passes (consistency, dead paths, typos, instruction contradictions, cold-session quality, hook consistency). Structural emitters live in this skill's `scripts/lint-structural-from-memory.sh` and `scripts/lint-structural-from-root.sh` (`references/lint-structural.md` is the pointer). New findings: `typo-heading` / `typo-token` / `method-stale` / `carrier-stale` / `hook-incomplete` / `opencode-legacy-plugin` / `learning-hidden` / `quality-unanswerable` / `live-dup-identity` / `overbroad-hint`.
- Memory method skip uses a write floor (rotten resume / user constraint / reusable lesson / closed why missing from the commit / shared blocker). A commit in Git does not skip the floor. Reusable lesson is fail closed (incident + 1-3 paths). Live user decisions beat leftover code for approach. Session Status is what the agent sees first this turn (`load:` / Next / Checkpoint). Always-load Markdown is `index.md` and `current.md`. Primary write is at most one file per event. `log.md` is not a session diary. Progress is optional. Idle `current.md` omits empty Blockers/Handoff. Sync no-ops without meaning and does not write `decisions.md` or learnings. Lint treats graph-tree folders as `graph-tree` and Progress as optional. Hooks still write ephemeral evidence only.
...
- Always-load `index.md` stays a short map of recall files (and rare gaps `AGENTS.md` does not already link), not a bibliography of `docs/`. `AGENTS.md` is the SoT for the project-docs map. `log.md` is a rolling window of recent closed outcomes (Git is the archive). Same calendar day and same `[type]`: update today's heading when the new outcome supersedes it. Lint warns `index-catalog` (max 3), `index-dup-agents`, `same-day-dup-log`, `log-unknown-type`, `decision-body-bloat`, `memory-ghost-docs`, `agents-docs-gap`. Consolidate may merge a false same-day heading, collapse superseded decision bodies, and patch the AGENTS docs map with confirmation.
- `/agent-memory init` and `update` patch `AGENTS.md` outside the memory block when `docs/` or an ADR index exists on disk and AGENTS does not already link it. They do not copy that map into `.agents/memory/`. Missing docs trees are not registered in memory. When a doc appears later, consolidate (or the turn that created it) adds the AGENTS pointer and strips the duplicate memory body. `decisions.md` is pointers plus a short local fallback, not a second ADR wiki; superseded entries collapse.
- Installed `instructions.md` names method rules in-file (learnings H2 fields, AGENTS docs map) instead of skill-only `references/*.md` paths that do not exist in consumer repos. **Cold session** (four questions a later agent must answer from always-on files) is the write quality bar; Q4 is the **done signal**. Resume rotten also fires when Validation is `_none_` / has no exact command. User constraint includes a flipped lasting assumption (this-branch flips go on active-work Assumptions). Duplicate rule: equivalent Insight still skip; opposite Insight in the target file gets `- Relates: contradicts` on the new H2 (`contradicts-unlinked` remains the leftover net). `lint` pass 7, `sync` Report, and the always-on block point at that heading. The write floor is the gate; format lives under _How to write_; start/stop point at those headings.
- `/agent-memory update` reads each harness `$hooksDir/.version` against the skill `metadata.version`. Matching stamp and complete scripts: skip installer commands (memory-only). Missing stamp, different version, or incomplete scripts: print the user-run refresh commands. The report offers `/agent-memory lint` as an optional check of the memory after the update. It names that `live-dup-identity`, `decision-lesson-dup`, and `overbroad-hint` wait for consolidate Pass A. Lint and sync do not slim `decisions.md` or invent learnings.
- `/agent-memory consolidate` always runs Pass A on the corpus (docs-map live entries, live+Source wiki bodies, live text that says superseded, incident-shaped decisions, missing `when editing:` on the decisions index line, live `memory-ghost-docs` including backtick `docs/…`, `double-injection` / `delegation-canary`) even when Pass B has nothing to prune from today's log. An open branch is not the current session. No-op only when both passes are empty. Pass A default is Apply (Defer only when evidence is missing). Live ghost/stale-live is Apply in the same run. If `decisions.md` is still > 200 lines, Pass A keeps collapsing remaining live wiki bodies to pointer-only in that invocation (User constraint may keep one Decision sentence). Redundant harness blocks are deleted in the same run. Prior-day log diary defaults to Trim. `incident-unpromoted` is per heading: one learning does not skip the rest. After promoting a learning, collapse the decision body (`decision-lesson-dup`). `lint --fix` deletes closed-placeholder resumes that are not the current git branch. At most three learnings per run, from decisions already on disk, not from the log diary.
