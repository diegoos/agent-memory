...
- Memory method: close the write loop — Checkpoint line is machine-only (`YYYY-MM-DD @ SHORT-SHA`, guidance off-line); sync/primary-write **must** consume eligible pending paths; Next step is product work only (never `/agent-memory …`); Progress must not replay `log.md`; lint adds `checkpoint-prose` / `stale-next-step` / `dup-progress-log` and skips dogfood `instructions.md`↔vendor `dup-exact`; bootstrap must not put skill commands in Next step. Fold into next bump: `sensitive` TEMPLATE + instructions + agent-block; `safe` sync/lint/bootstrap refs.
- Memory method: Checkpoint line is plain `YYYY-MM-DD @ SHORT-SHA` (no backticks around date/sha) — TEMPLATE, sync, and instructions aligned; lint accepts legacy backtick form with `checkpoint-backticks` warning and skips placeholder `./file` link checks; active-work References use `../../../` for repo-root docs; `when editing:` syntax example moved into a fenced block.
- Memory method: primary-write triggers (end of durable turn, after commit, before compact, Status stale/pending); concise _How to write_ guidance; sync prefers CHANGELOG / `git log` subjects / Validation as meaning sources; sync consumes `session_touched_files` via `agent-memory-consume-evidence.sh`; learn must report skips (dedupe/gate) and propose path-scoped `when editing:` when Evidence is 1–3 concrete paths; lint `evidence-stale-uncleared` + `pending-doc-met`; consolidate handles met `pending-doc` and mixed-type log bullets; agent-block + sessionStart Action line aligned. Next bump: `sensitive: instructions.md` + `active-work/TEMPLATE.md` + harness agent-block refresh; `safe:` consume-evidence script (re-run hooks installer).
