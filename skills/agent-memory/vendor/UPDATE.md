...
- sensitive: `init` / `update` / `bootstrap` / `consolidate` / `sync` — docs map on `AGENTS.md` (`references/docs-map.md`); sync does not add Canonical docs links; consolidate may patch AGENTS map bullets outside the agent-memory block and strip duplicate memory bodies when a doc appears.
