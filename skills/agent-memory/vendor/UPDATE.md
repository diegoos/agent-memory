...
- sensitive: `instructions.md` — catch-up and harness parity: print-evidence / consume-evidence; do not Read `.hook-sync-state`. Show diff and confirm. Preserve project-specific notes. Method rules for learnings H2 and the AGENTS docs map live in-file (no skill-only `references/*.md` pointers in the installed copy).
...
- safe: `SKILL.md` — `update` allowed-tools include `decisions.md` and mirror paths for graph reshape only; `AGENTS.md` for docs-map patches (`references/docs-map.md`); sync still must not write decisions/learnings. Help/routing name mirror delete + AGENTS map.
- sensitive: `index.md` / `instructions.md` — Canonical sources list a path only when `AGENTS.md` does not already link it and a cold session needs it before choosing a recall file; missing `docs/` / ADR trees stay unlisted; superseded decisions collapse to heading + Status + `Superseded by:`; no ADR-index pointer in memory when the tree is absent. Show diff and confirm.
- safe: `update` — classify each installed harness via `$hooksDir/.version` vs skill `metadata.version` plus the five shared scripts (OpenCode: plugins + `safe-script.ts`). Print installer commands only when **stale** (older SemVer or incomplete). Matching or **later** stamp = memory-only report (`installer skip`; no downgrade). Report may offer `/agent-memory lint` to check memory (optional).
- safe: `lint` — findings `decision-canonical-dup` / `decision-docs-map` / `decision-stale-live` / `incident-unpromoted` (per heading) / `decision-hidden` (aggregate: any live decision names `src/` or a `*.config.*` path while the index `decisions.md` line lacks `when editing:`) / `decision-lesson-dup` (Relates on the decision **or** a learnings `decisions.md#slug` back-link; not prose H2 mentions) / `closed-placeholder-resume` / `wrong-cwd:` (structural script not run from the documented cwd — memory: `.agents/memory/`; root: project root); `dup-progress-log` at >5 Progress bullets; `live-dup-identity` docs-layout heuristic; Overbroad includes `src/pages/**`. `relates-missing` `#fragment` is a heading slug (cached per target). Root `stale-resume` accepts any valid `Checkpoint:` matching HEAD. Orphan basename probes use literal `grep -F`. `decisions.md` > 200 maps to consolidate Pass A (Reference), not a learnings split. `lint --fix` may delete closed-placeholder resumes that are not the current git branch. Resolve skill-dir to an absolute path, then run `<absolute-skill-dir>/scripts/lint-structural-from-memory.sh` from `.agents/memory/`.
- sensitive: `consolidate` — two passes: **A corpus** (always) and **B closed log**. Report **no-op** only when both are empty. Open branch ≠ current session. Empty Pass A while those lint IDs exist is a failed consolidate. Pass A default **Apply** (Defer only with missing-evidence reason). Prior-day log headings default **Trim** (not Retain on diary). At most 3 Learning promotions per run, from incident-shaped decisions only (not log diary); one existing H2 does not skip other `incident-unpromoted` headings. Collapse `decision-lesson-dup`. Delete `closed-placeholder-resume` when not the current git branch. Show plan; confirm each diff.
- sensitive: `instructions.md` — live `Source:` to spec/ADR stays a pointer; live + “superseded by” is a bug; identity is the approach noun (not a unique heading); `when editing:` hints are Evidence path literals (not `src/pages/**`); in-turn Reusable lesson stays fail-closed; `/agent-memory consolidate` may promote incident-shaped decisions already on disk. Show diff and confirm.
- sensitive: harness agent-memory block + `instructions.md` _When stopping_ — last assistant line `Memory: skip` or `Memory: <file>` after a turn that changed repo files (not a write). Show block diff; confirm.
...
- safe: hooks — Status `when editing:` covering `src/pages/**…` / `src/<seg>/**`. Sync checkpoint lists tracked dirty only (no untracked walk). Session-id fallback binds the first `"session_id"` key. **User must re-run the hooks installer** (shared scripts).
- sensitive: `instructions.md` — Validation max 5 bullets (closure command; historical reviews in `log.md`). **Cold session** four questions are the write quality bar; Q4 is the **done signal**. Resume rotten also when Validation is `_none_` / has no exact command. User constraint includes a flipped lasting assumption; this-branch flips go on active-work Assumptions. Duplicate rule: equivalent Insight still skip; opposite Insight in the target file → `- Relates: contradicts` on the new H2 (a different Use when is not a conflict). Slim: write floor is the gate; format lives under _How to write_; start/stop point at those headings (same behaviour). Write-floor **Winner** (first yes): User constraint → Reusable lesson → Closed why → Resume rotten → Shared blocker. Show diff and confirm.
- sensitive: `consolidate` — Pass A Apply on live `memory-ghost-docs` (missing backtick `docs/…`); `decisions.md` **non-empty** > 200 is a Pass A candidate even with no other ID; second Pass A in the same run (even with no earlier corpus diffs) until non-empty ≤ 200 or remaining live headings are pointer-only (blank lines do not count; Report `decisions.md non-empty N (budget 200)`; leftover live Context/Why while N > 200 is a failed consolidate). Pass A also deletes `double-injection:` / `delegation-canary:` redundant blocks in that run. Pass B trims `dup-validation:`. Show plan; confirm each diff.
