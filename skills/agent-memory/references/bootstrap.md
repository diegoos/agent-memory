...
4. **Synthesize and write (inventory-first, skip empties).** Wait for all three, then fill memory **without copying docs**:
   ...
   - Create `learnings.md` (or a topic split) only when at least one fact passes the gate (reusable, undocumented, non-obvious, evidenced, no secrets). Use the H2 learning/pitfall format in `references/learn.md`. Documented-vs-actual layout or AGENTS/README closure gaps that pass the gate **must** become learnings (not log-only inventory): include `pending-doc` **and** `Invalidate when: <concrete doc condition>`. Skip facts already clear from a linked canonical one-liner; prefer quality over volume (merge related tooling nits when one Invalidate condition covers them). Link every new learnings file from `index.md`. When Evidence paths are clear and globs stay within the `when editing:` contract, add a narrow hint on the learnings index line (AskQuestion if unsure). Prefer one `learnings.md` on first bootstrap unless a single theme dominates. A cold session after bootstrap should find docs on `AGENTS.md` and gaps as learnings — not a vision/architecture wiki.
...
5. **Report.** Group clearly:
   ...
   - **Ignored** — transient or already covered by a source. Tell the user to run `/agent-memory sync` at checkpoints. After hooks are installed for this harness, **re-run sync** (or `sync --auto` / `sync auto`) so blockers/evidence catch up — name the harness (Cursor `.mdc` vs OpenCode `AGENTS.md` + `.opencode/plugins/`). Prefer `/agent-memory consolidate` later for **closed**-session **prune**; same-day consolidate after bootstrap is **report-only** at best (pending-doc / commit hygiene) — founding log headings are current session (do not expect Discard/Trim).
   **Done when:** existing project-docs indices are on `AGENTS.md` (or each omission was an explicit skip); every gate-passing gap is a learning with evidence (not a wiki file); `log.md` has one synthesis heading for this bootstrap.
