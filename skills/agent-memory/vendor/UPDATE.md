...
- safe: `lint --fix` may refresh `stale-resume:` Checkpoint (today @ HEAD short SHA) and trim `dup-progress-log:` / `dup-validation:` to ≤5 bullets (confirm). `overbroad-hint:` includes covering `src/pages/**…` and `src/<seg>/**` trees. `live-dup-identity:` docs-layout heuristic matches `reorganiz` / `docs layout` / `docs suite` / `docs/specs` / `docs tree` (not a named third-party docs skill). `decision-lesson-dup:` / `memory-ghost-docs:` stay consolidate Pass A (default Apply; identity successor supersede+collapse; live decision backtick `docs/…` missing on disk). Update report: `--fix` does not slim `decisions.md`. When installed SemVer is ahead of the skill, report that `.version` stays and stable-core migrations will not run (always-on gitignore / graph reshape / blocks still run).
- safe: hooks — `hooks/agent-memory-hooks/` derives install-site like harness dirs (stale `*_PROJECT_DIR` / `GITHUB_WORKSPACE` must not retarget state). Status `when editing:` uses lint Overbroad denylist (covering `src/pages/**…` / `src/<seg>/**`); `*` does not cross `/`. **User must re-run the hooks installer** (shared scripts).
- sensitive: `instructions.md` — Validation max 5 bullets (closure command; historical reviews in `log.md`). Show diff and confirm.
- sensitive: `consolidate` — Pass A Apply on live `memory-ghost-docs` (missing backtick `docs/…`); second Pass A in the same run when `decisions.md` still > 200 and live ghost/stale-live remain. Pass B trims `dup-validation:`. Show plan; confirm each diff.
