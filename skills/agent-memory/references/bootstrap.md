...
4. **Synthesize and write (inventory-first, skip empties).** Wait for all three, then fill memory **without copying docs**:
   ...
   | Source       | Writes to                                                     |
   | ------------ | ------------------------------------------------------------- |
   | A (sources)  | `index.md` → _Canonical project sources_ (short map)          |
   ...
   - Keep every `index.md` source line to: link + what it owns + when to read. Prefer agent file + README + docs/ADR index. Do not list every config (`package.json`, SECURITY, skill) unless a cold session must open it before choosing a file.
