...
Shared bash hooks validate external session binding IDs and take the first valid among `session_id` / `conversation_id` / `sessionId` / `conversationId` / `composer_id`. They prefer the project env or install-site root over harness stdin `cwd`. Vendor scripts under `hooks/agent-memory-hooks/` derive that package/repo root the same way as `<project>/.cursor/hooks` (stale `CURSOR_PROJECT_DIR` / `GITHUB_WORKSPACE` must not retarget `.hook-sync-state`). They fail closed and write nowhere when the env workspace's harness hooks resolve into another project, or when env has a divergent wrapper or entrypoint. Otherwise they prefer install-site over a mismatched env that has no retarget.
