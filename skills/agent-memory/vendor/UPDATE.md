...
- safe: OpenCode plugin install path — `.opencode/plugins/` (OpenCode auto-load; was singular `.opencode/plugin/`) + copy `safe-script.ts` beside `agent-memory.ts`; installer removes legacy singular files. **User must re-run** `install hooks opencode` then restart OpenCode.
...
- sensitive: `instructions.md` — primary-write triggers; concise _How to write_; sync meaning sources; **must consume** pending paths when eligible; Checkpoint plain `YYYY-MM-DD @ SHORT-SHA` (no backticks / trailing prose); Next step product-only; Progress must not replay log; consolidate never empties current-session log; fenced `when editing:` shape. Show full diff and confirm. Preserve project-specific notes.
- sensitive: harness agent-memory block — primary-write triggers; consume-evidence in-turn when pending paths covered / on catch-up. Confirm per file.
- sensitive: `active-work/TEMPLATE.md` — Checkpoint plain `YYYY-MM-DD @ SHORT-SHA` on its own line (guidance off-line); Next step = product action only; Progress must not replay log; Validation prefers full closure; References `../../../` example. For existing `active-work/*.md`: strip backticks and trailing Checkpoint prose; fix `../../` → `../../../` when needed (show diff, confirm).
- safe: `sync` / `learn` / `lint` / `consolidate` / `bootstrap` references — meaning sources; **required** consume when eligible; learn skip report + `when editing:` from Evidence; `evidence-stale-uncleared` / `pending-doc-met` / `checkpoint-prose` / `stale-next-step` / `dup-progress-log` / `empty-log` / `empty-log-after-scaffold` / `hook-state-absent`; lint skips `./file` and angle-bracket Shape placeholders (`learnings-<topic>.md`); skip dogfood instructions↔vendor `dup-exact`; consolidate current-session / never-empty-log / Trim / Progress follow-up / Report names retained founding log; sync prefers refining today's heading and suggests consolidate only for closed-session noise; Validation prefers full closure command; bootstrap **one** synthesis log heading + Next step product-only; mixed-type log / met pending-doc.
- safe: hooks — `agent-memory-consume-evidence.sh` (clears `session_touched_files` only; skip under lock fail-open + compare-and-swap); sessionStart Status uses `parse_checkpoint_sha`, tracked `dirty` flag, Action with repo-relative consume path; stdin binding also accepts `conversationId` / `composer_id`; Checkpoint parse tolerates legacy backticks; OpenCode `isValidBindingId` rejects `__no_id__`; installer writes `.version` via temp+mv (no symlink follow). **User must re-run the hooks installer** (four shared scripts).
