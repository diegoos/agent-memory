...
- sensitive: `active-work/TEMPLATE.md` — Checkpoint plain `YYYY-MM-DD @ SHORT-SHA` on its own line (guidance off-line); Next step product-only guidance **off-section** (never under `## Next step`); Progress must not replay log; Validation prefers full closure; References `../../../` example; copy/create strips section blurbs (keep `##` + `-` bullets). For existing `active-work/*.md`: strip backticks and trailing Checkpoint prose; strip leftover TEMPLATE section blurbs; fix `../../` → `../../../` when needed (show diff, confirm).
- safe: `sync` / `learn` / `lint` / `consolidate` / `bootstrap` references — meaning sources; **required** consume when eligible; learn skip report + `when editing:` from Evidence; `evidence-stale-uncleared` / `pending-doc-met` / `checkpoint-prose` / `stale-next-step` (action bullets only) / `dup-progress-log` / `empty-log` / `empty-log-after-scaffold` / `hook-state-absent`; lint skips `./file` and angle-bracket Shape placeholders (`learnings-<topic>.md`); skip dogfood instructions↔vendor `dup-exact`; consolidate current-session / never-empty-log / Trim / Progress follow-up / Report names retained founding log; sync prefers refining today's heading and suggests consolidate only for closed-session noise; Validation prefers full closure command; bootstrap **one** synthesis log heading + Next step product-only; sync/bootstrap strip TEMPLATE section blurbs on create/refresh; mixed-type log / met pending-doc.
