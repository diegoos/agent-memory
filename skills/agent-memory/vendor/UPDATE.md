...
- safe: `update` — classify each installed harness via `$hooksDir/.version` vs skill `metadata.version` plus the five shared scripts (OpenCode: plugins + `safe-script.ts`). Print installer commands only when **stale**. Matching stamp = memory-only report (`installer skip`). Report may offer `/agent-memory lint` to check memory (optional).
