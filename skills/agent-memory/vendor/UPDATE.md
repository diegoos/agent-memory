...
- safe: `lint` — findings `decision-canonical-dup` / `decision-docs-map` / `decision-stale-live` / `incident-unpromoted` / `decision-hidden`; `decisions.md` > 200 maps to consolidate Pass A (Reference), not a learnings split.
- sensitive: `consolidate` — two passes: **A corpus** (always) and **B closed log**. Report **no-op** only when both are empty. Open branch ≠ current session. Empty Pass A while those lint IDs exist is a failed consolidate. At most 3 Learning promotions per run, from incident-shaped decisions only (not log diary). Show plan; confirm each diff.
- sensitive: `instructions.md` — live `Source:` to spec/ADR stays a pointer; live + “superseded by” is a bug; in-turn Reusable lesson stays fail-closed; `/agent-memory consolidate` may promote incident-shaped decisions already on disk. Show diff and confirm.
