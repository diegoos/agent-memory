...
4. **Apply, conservatively:**
   - **Automatic (no prompt):**
     ...
     - Ensure `.agents/memory/.gitignore` exists and matches this skill's `vendor/memory/.gitignore` (Read vendor, Write destination). Hosts often hide dotfiles from `Glob` — do **not** rely on directory listings. If the installed file is missing or differs only by missing `.hook-sync-state` (or legacy `.cursor-hook-state`), replace it without prompting. If the user added extra ignore rules, **merge** (keep theirs + ensure `.hook-sync-state` is listed) and treat that merge as **sensitive** (show diff, confirm).
