...
- Structural lint `overbroad-hint:` when an `index.md` `when editing:` glob is on the Overbroad denylist (`src/pages/**` and the rest). Consolidate Pass A replaces those globs with Evidence path literals (default Apply). Companions do not redeem a denylist glob.
...
- `/agent-memory update` and `init` stamp `.agents/memory/.version` from `SKILL.md` `metadata.version`, not a newer drafted UPDATE heading. They do not stamp downward when installed is a later SemVer than the skill (RC consumer that already wrote `0.2.1`).
- `lint --fix` may refresh `stale-resume:` Checkpoint (today @ HEAD) and trim `dup-progress-log:` Progress to ≤5 bullets (confirm). It still does not slim `decisions.md` or rewrite overbroad index globs.
- Vendor `hooks/agent-memory-hooks/` is an install-site root (same stale-env preference as `.cursor/hooks`). Session Status `when editing:` matching uses the lint Overbroad denylist and gitignore-style `*` (does not cross `/`). **User must re-run the hooks installer** (shared scripts).
