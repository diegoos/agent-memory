...
- `tests/version-parity.sh` accepts npm prerelease versions (`0.2.1-rc.2`). CHANGELOG/UPDATE headings and GitHub pins stay on the last released tag until the RC is published. CLI `compareSemver` orders prerelease below the same core (so `0.2.1-rc.2` is not treated as equal to `0.2.1`).
...
- `/agent-memory update` reads each harness `$hooksDir/.version` against the skill `metadata.version`. Matching stamp and complete scripts: skip installer commands (memory-only). Missing stamp, different version, or incomplete scripts: print the user-run refresh commands. The report offers `/agent-memory lint` as an optional check of the memory after the update. Lint and sync do not slim `decisions.md` or invent learnings.
