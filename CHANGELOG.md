...
- Structural awk reads paths from `ENVIRON` (not `-v`, which interprets `\\n` in names). `relates-missing` `#fragment` matches `memory_slug` of target H2s (cached per target file). Learnings back-links match a whole slug via `decisions.md#…`, not prose H2 mentions or prefixes. `decision-hidden` fires once when any live decision names `src/` or `astro.config` and the index `decisions.md` line lacks `when editing:`. Orphan basename probes use `grep -F`. Memory-cwd and root-cwd emitters emit `wrong-cwd:` when not run from the documented cwd. Root `stale-resume` accepts any valid `Checkpoint:` line that matches HEAD. Grep operands use `-e`/`--`. Lint-structural tells the agent to resolve skill-dir as an absolute path before `cd` into `.agents/memory/`.
...
- Installed `instructions.md` names method rules in-file (learnings H2 fields, AGENTS docs map) instead of skill-only `references/*.md` paths that do not exist in consumer repos.
