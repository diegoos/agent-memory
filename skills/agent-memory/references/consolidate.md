...
3. **Collect candidates (read-only).** Two lists. Never skip Pass A because Pass B is empty.
   ...
   **Pass A — corpus** (exclude nothing for these IDs; they are not log prune):
   ...
   - Two `Status: live` entries on the same identity (append without supersede) — keep one live successor (`live-dup-identity:`). **Identity is the approach noun** (docs tree layout, TypeScript major, named-slot wrap), not a unique heading. Two live entries that pick different values for the same noun and have no `Superseded by:` / `Relates: supersedes` between them are one identity — newest stays live; older get `Status: superseded` + collapse.
   - Path-scoped learnings / topic splits listed in `index.md` **without** a `when editing:` hint when Evidence names concrete paths (lint stale/missing hint). Live `decisions.md` that names 1–3 repo-relative paths while the index `decisions.md` line has no hint (`decision-hidden:`). Index `when editing:` glob on the Overbroad denylist (`overbroad-hint:`) — replace those globs with Evidence path literals from the linked recall file (max 3). A narrow companion glob does not redeem a denylist glob. Default **Apply**.
   - **`live-dup-identity:` / `decision-stale-live:`** — newest live successor stays; older get `Status: superseded` + collapse in the same Apply. A still-useful older body is not a Defer reason. Live identity whose named doc path is missing (`memory-ghost-docs`, vanished `validations.md`) → supersede toward the live successor or drop the ghost link (Apply).
...
6. **Apply in safe order** — confirm each diff (approve / skip / abort):
   ...
   6. **Progress follow-up** — offer to Trim `dup-progress-log:` on the current branch file, or refresh Progress so it does not point at removed log headings after an approved closed-session log Discard/Trim. Skip this AskQuestion only when neither applies.
