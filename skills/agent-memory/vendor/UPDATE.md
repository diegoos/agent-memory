...
- safe: `SKILL.md` — `update` allowed-tools include `decisions.md` and mirror paths for graph reshape only; `AGENTS.md` for docs-map patches (`references/docs-map.md`); sync still must not write decisions/learnings. Help/routing name mirror delete + AGENTS map. After the loaded command's Report, last assistant line `Memory: skip`. Always-on block names triggers; _Harness parity_ stays a pointer into `instructions.md`.
...
- sensitive: harness agent-memory block + `instructions.md` _When stopping_ — last assistant line `Memory: skip` or `Memory: <file>` after a turn that changed repo files (not a write). Injected block names the same Winner order. Show block diff; confirm.
