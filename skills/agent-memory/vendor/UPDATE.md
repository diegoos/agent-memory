...
- sensitive: `active-work/TEMPLATE.md` — Checkpoint plain `YYYY-MM-DD @ SHORT-SHA` on its own line (guidance off-line); Next step = product action only; Progress must not replay log; References `../../../` example. For existing `active-work/*.md`: strip backticks and trailing Checkpoint prose; fix `../../` → `../../../` when needed (show diff, confirm).
- safe: `sync` / `learn` / `lint` / `consolidate` / `bootstrap` references — meaning sources; **required** consume when eligible; learn skip report + `when editing:` from Evidence; `evidence-stale-uncleared` / `pending-doc-met` / `checkpoint-prose` / `stale-next-step` / `dup-progress-log` / `empty-log` / `empty-log-after-scaffold`; skip dogfood instructions↔vendor `dup-exact`; consolidate current-session / never-empty-log / Trim / Progress follow-up gates; mixed-type log / met pending-doc; bootstrap **one** synthesis log heading + Next step product-only; sync prefers refining today's heading.
