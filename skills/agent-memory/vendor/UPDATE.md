...
- sensitive: `instructions.md` — primary-write triggers; concise _How to write_; sync meaning sources + consume pending paths; log heading type hygiene; Checkpoint without backticks; fenced `when editing:` shape. Show full diff and confirm. Preserve project-specific notes.
...
- sensitive: `instructions.md` — must-consume on primary-write/sync; Next step product-only; Checkpoint line hygiene; Progress vs log; consolidate never empties current-session log. Show full diff and confirm. Preserve project-specific notes.
- sensitive: harness agent-memory block — consume in-turn when pending paths covered. Confirm per file.
- safe: hooks — `agent-memory-consume-evidence.sh` (clears `session_touched_files` only); sessionStart Status adds Action + dirty; stdin binding also accepts `conversationId` / `composer_id`; Checkpoint parse tolerates legacy backticks. **User must re-run the hooks installer** (four shared scripts).
