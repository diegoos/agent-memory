...
4. **Synthesize and write (inventory-first, skip empties).** Wait for all three, then fill memory **without copying docs**:
   ...
   - Append **one** synthesis heading to `log.md` for the whole bootstrap (init scaffold + inventory + gaps) — e.g. `## [YYYY-MM-DD] [docs] bootstrap source inventory` — with bullets for sources indexed / learnings created / gaps reported. Do **not** open a second `[ingest]` / init-only heading for the same calendar day; fold init into this heading. Keep bullets short; durable gaps belong in `learnings.md`, not as Progress replay.
...
5. **Report.** Group clearly:
   ...
   - **Ignored** — transient or already covered by a source. Tell the user to run `/agent-memory sync` at checkpoints. Prefer `/agent-memory consolidate` later for **closed**-session noise — not immediately after bootstrap on the same day (founding log headings are current session).
