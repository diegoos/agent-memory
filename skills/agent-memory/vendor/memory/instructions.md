...
**Hooks own ephemeral evidence only:** session id binding; branch cache; session-cumulative touched paths; `last_processed_head` / commit range markers. Four flat scripts beside the install site (`agent-memory-common.sh`, `agent-memory-sync.sh`, `agent-memory-session.sh`, `agent-memory-consume-evidence.sh`). `agent-memory-consume-evidence.sh` clears `session_touched_files` after the agent records semantic outcomes (compare-and-swap; preserves binding). sessionStart may **read** active-work Markdown for Status injection (Checkpoint, pending paths, Action) — read-only, never write. Hooks never create or edit Markdown under `.agents/memory/`.
