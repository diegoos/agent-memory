...
- **Git `pre-commit`:** also inherits the parent environment, except it **unsets** `AGENT_MEMORY_SESSION_ID`, `CURSOR_SESSION_ID`, and `GEMINI_SESSION_ID` before sync and sets `AGENT_MEMORY_HOST=git` (drops inherited host labels such as stale `opencode` — pre-commit has no harness stdin session id; stale shell state must not rebind `.hook-sync-state`).
