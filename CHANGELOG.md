...
- Git `post-commit` stamps `last_processed_head` to the new HEAD and drops `session_touched_files` entries that are in that commit and no longer dirty. It never writes Markdown, never blocks, and fails open. Copy it next to `pre-commit` (refresh `.git/hooks/post-commit` and the shared scripts).
...
- `/agent-memory update` reads each harness `$hooksDir/.version` against the skill `metadata.version`. Matching stamp and complete scripts: skip installer commands (memory-only). Missing stamp, different version, or incomplete scripts: print the user-run refresh commands. The report offers `/agent-memory lint` as an optional check of the memory after the update.
