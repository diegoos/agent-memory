...
| `consolidate`   | Pass A Apply; same-run slim if `decisions.md` > 200; redundant block delete; Pass B prior-day Trim; confirm; no `--auto` / `auto` | `references/consolidate.md`   |
...
| `/agent-memory consolidate`   | Pass A (decisions/index/learnings/redundant harness block) even when Pass B log prune is empty; default Apply; if `decisions.md` stays >200, keep collapsing live wiki bodies in the same run; prior-day log Trim (not Retain); confirm each diff; no `--auto` / `auto`. `lint --fix` / `lint fix` and `sync` do not replace this. |
...
- Daily write lives in `.agents/memory/instructions.md` (write floor; skip when all no; **cold session** four questions). `sync` when Status is stale **and** there is meaning (`sync --auto` / `sync auto` for routine flushes; **must consume** pending paths when meaning covers them). `learn [>topic] <clue>` only for explicit capture. `consolidate` to slim (Pass A always; same-day after bootstrap is report-only for Pass B; `lint --fix` / `lint fix` and `sync` do not slim `decisions.md`). `lint` for health; `update` for scaffolding and leftover `vision.md` / `domains/*` mirrors (docs stay on `AGENTS.md`). Hook installer commands print only when `$hooksDir/.version` is missing, differs from this skill, or scripts are incomplete.
