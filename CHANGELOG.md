...
- `tests/version-parity.sh` accepts npm prerelease versions (`0.2.1-rc.1`). CHANGELOG/UPDATE headings and GitHub pins stay on the last released tag until the RC is published. CLI `compareSemver` orders prerelease below the same core (so `0.2.1-rc.1` is not treated as equal to `0.2.1`).
