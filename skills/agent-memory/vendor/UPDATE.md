...
- sensitive: `active-work/TEMPLATE.md` — Checkpoint plain `YYYY-MM-DD @ SHORT-SHA` (no backticks around date/sha); References note `../../../` for repo-root links. For existing `active-work/*.md`: strip backticks on Checkpoint; fix `../../` → `../../../` for root docs when needed (show diff, confirm).
- safe: `sync` / `learn` / `lint` / `consolidate` references — meaning sources; consume-evidence step; learn skip report + `when editing:` from Evidence; `evidence-stale-uncleared` / `pending-doc-met`; mixed-type log / met pending-doc consolidate actions; lint Checkpoint backtick tolerance + placeholder link skip.
