...
- sensitive: `consolidate` — Pass A Apply on live `memory-ghost-docs` (missing backtick `docs/…`); second Pass A in the same run when `decisions.md` still > 200 until ≤ 200 or remaining live headings are pointer-only (not only leftover ghosts). Pass A also deletes `double-injection:` / `delegation-canary:` redundant blocks in that run. Pass B trims `dup-validation:`. Show plan; confirm each diff.
